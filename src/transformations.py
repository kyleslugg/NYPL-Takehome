...
def load_311(loader: SocrataLoader):
    ...
    cases['date'] = pd.to_datetime(cases['created_date']).apply(lambda x: x.date())
    cases = gpd.GeoDataFrame(cases,
                             geometry=gpd.points_from_xy(cases['x_coordinate_state_plane'], cases['y_coordinate_state_plane'], crs='2263'))
...
def load_permits(loader: SocrataLoader):
    ...
    permits['parkingheld'] = permits['parkingheld'].apply(lambda x: re.sub('\s+', ' ', x))
    permits['parkingheld'] = permits['parkingheld'].apply(lambda x: [st.strip() for st in x.split(',')])
...
def load_intersection_mapper(intersection_mapper_path: pathlib.Path, intersection_locations: pathlib.Path, refresh=False):
    ...
    if refresh or not intersection_mapper_path.exists():
        ...
        node_street_pairs = gpd.GeoDataFrame.from_file(intersection_locations, crs='2263')
        ...
        pairs_to_points = node_street_pairs.explode('street_combos')[['street_combos', 'geometry']]
        pairs_to_points.to_file(intersection_mapper_path, driver='GeoJSON', index=False, engine='pyogrio')
        print("Intersection mapper saved to disk.")

    if pairs_to_points is None:
        print("Loading intersection mapper from disk.")
        pairs_to_points = gpd.GeoDataFrame.from_file(intersection_mapper_path, crs='2263')

    print("Loaded intersection mapper.")

    pairs_to_points = pairs_to_points[pairs_to_points['geometry'].is_valid]
    pairs_to_points['street_combos'] = pairs_to_points['street_combos'].apply(lambda x: tuple(x))
...
def parking_loc_to_intersections(row):
    ...
    [main_st, side_streets] = [st.strip() for st in parking_loc.split('between')]
    [ss_from, ss_to] = [st.strip() for st in side_streets.split('and')]

    row['from_intersection'], row['to_intersection'] = (main_st, ss_from), (main_st, ss_to)
    return row
...
def find_intersection_midpoint(row, locator_df, start_int_col='from_intersection', end_int_col='to_intersection', locator_geometry_col='geometry', locator_street_col='street_combos'):
    try:
        start_geom = locator_df[
            locator_df[locator_street_col] == row[start_int_col]
            ].iloc[0][locator_geometry_col]
        end_geom = locator_df[
            locator_df[locator_street_col] == row[end_int_col]
            ].iloc[0][locator_geometry_col]
...
def get_shooting_days_locations(permits: pd.DataFrame, intersection_point_mapper: gpd.GeoDataFrame):
    ...
    days_locations = locs.explode('date')[['midpoint', 'date']]
...
def tax_block_date_matrix(tax_block_gdf: gpd.GeoDataFrame, tax_block_ids, tax_block_keep_fields, start_date, end_date, date_column):
    ...
    bb = tax_block_gdf[tax_block_ids + tax_block_keep_fields].groupby(tax_block_ids).first().reset_index()
    bb[date_column] = [pd.date_range(start_date, end_date, freq='D') for _n in range(len(bb))]
...
def points_by_day_tax_block(point_gdf: gpd.GeoDataFrame, tax_block_gdf: gpd.GeoDataFrame, tax_block_date_matrix: pd.DataFrame, tax_block_ids, point_date_field, point_count_field_id):
    ...
    block_day_counts = joined.groupby(tax_block_ids+[point_date_field]).count().iloc[:, :1]
